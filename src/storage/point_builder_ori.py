...
class PointBuilder:
    ...
    def build_point(
        ...
        # 2. 组装行李箱 (Payload)
        # 💡 架构师技巧：为了防止 Payload 撑爆内存，建议只存抠图的 Base64，原图太大了只存路径
        crops_base64 = [PointBuilder.encode_image_to_base64(p) for p in crop_paths]
        ...
        payload = {
            ...
            "extracted_crops_b64": crops_base64 # 前端画 UI 直接用这个
        }
